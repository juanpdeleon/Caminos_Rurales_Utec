...
def preocess_input_video(input_dir, output_dir, backup_dir, Log_dir):
    ...
    camiones_dir = os.path.join(output_dir, "camiones")
    ...
    os.makedirs(camiones_dir, exist_ok=True)
    ...
    for archive in content:
        ...
        status = count_specific_classes(
            ...
            [0,1, 2, 3,4,5], cropped_box, saved_json, archive
        )
...
def count_specific_classes(video_path, output_video_path, model_path, classes_to_count, cropped_box, saved_json, archive):
    ...
    line1_y = 250
    line2_y = 300
    line_points_max = [(10, line1_y), (1300, line1_y)]  # Línea principal de detección

    counter = solutions.ObjectCounter(
        show=True, region=line_points_max, model=model_path,
        classes=classes_to_count, verbose=False, line_thickness=1
    )
    ...
    while cap.isOpened():
        ...
        # Procesar el frame con YOLOv11
        results = counter.model(im0)  # Obtener resultados de YOLOv11
        im0 = counter.count(im0)  # Dibujar contadores y bounding boxes

        video_writer.write(im0)
        cv2.line(im0, (10, line1_y), (1300, line1_y), (255, 100, 255), 1)  # Dibujar línea de detección

        frame_count += 1

        # Obtener class_ids, boxes y track_ids desde los resultados de YOLOv11
        for result in results:
            ...
            track_ids = result.boxes.id.cpu().numpy().astype(int) if result.boxes.id is not None else []  # IDs de seguimiento

            for track_id, box, class_id in zip(track_ids, boxes, class_ids):
                x1, y1, x2, y2 = box
                centroid = ((x1 + x2) // 2, (y1 + y2) // 2)
                tipo_vehiculo = str(class_id) # CLASES.get(class_id, "unknown")

                if line1_y < centroid[1] < line2_y:  # Rango de detección ajustado
...
def save_json_file(list_data, saved_json, file_name):
    ...
    file_name_path = os.path.join(saved_json, f"{file_name}.json")
    datos_json = {
    ...
    with open(file_name_path, "w") as archivo:
        json.dump(datos_json, archivo, indent=4)

    print(f"Archivo JSON creado: {file_name_path}")
    return True