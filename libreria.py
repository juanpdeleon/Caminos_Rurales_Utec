    ...
    if not os.path.exists(input_dir):
      print("El directorio original no existe..")
        ...
        if not os.path.exists(output_dir):
            ...
            os.makedirs(cropped_box)    #Directorio del boundingbox recortado
            os.makedirs(saved_frame)    #Directorio del frame completo de la detecction
            os.makedirs(backup_dir)     # directorio a donde se mueve el video procesado
            ok.makedirs(Log_dir)        # Directorio a dode se guardara un archivo con toda los informes del algoritmo
        ...
        #Carga el sistema principal
            for archive in content:
                ...
                status=count_specific_classes(path_video_file, "output_specific_classes.avi", "yolo11n.pt", [1,2,3,5,7], cropped_box, saved_frame,archive)              
                status = True
                if status:
                    # os.remove(archive)
...
#======================================================================================================================================
def count_specific_classes(video_path, output_video_path, model_path, classes_to_count, cropped_box, saved_frame,archive):
                ...
                if (centro > 350 and centro <400):  # Direccion de deteccion arrriba y abajo 
                        if track_id not in objects_crossed:
                          ...
                          #  file_data = open(output_csv, "a")
                           ...
                           # file_data.close()
                            save_json(list_data,name_archive_crop)
...
def save_json(list_data,file_name): # recibo como parametros los datos en una lista y el nombre del archivo
    ...
    with open(file_name+".json", "w") as archivo:
